const sia = {
  ...
  initMap: function() {
    $('#map-container').slideToggle(function() {
      $(this).is(':visible') ? $('#map-toggle').text('(Hide map!)') : $('#map-toggle').text('(Show map!)');
    });
  ...
  scrollGallery: function() {
    // Scrolls vertically to #quote-hired
    $('html, body').animate({ scrollTop: $('#quote-hired').offset().top - 150 }, 'slow');

    // Scrolls horizontally inside gallery
    const leftPos = $('#quote-hired')[0].scrollWidth;
    $('#quote-hired').animate({scrollLeft: leftPos}, 'slow');
  },
